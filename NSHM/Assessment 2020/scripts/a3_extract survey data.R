...
outpath <- "NSHM/Assessment 2020/"


#---------------------------------------------------------------------------------------#
# Extract and select hauls, explore HH data -----
...
if(!file.exists(paste(datpath,datafileHL,sep=""))){
  survey_all_hl <- getHHdata(survey=surveys, year=years, quarter=3)
  save(survey_all_hl, file=paste(datpath,datafileHL,sep="")) #save raw data
} else {
  load(paste(datpath,datafileHL,sep=""))
}




aphia    <- findAphia("Trachurus trachurus", latin=TRUE)
tsn
