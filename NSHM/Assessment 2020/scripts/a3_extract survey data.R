...
datpath <- "NSHM/Assessment 2020/data/survey/"
...
# Rename surveys
survey_hl$Survey <- with(survey_hl, ifelse(Survey %in% "FR-CGFS","CGFS","IBTS"))
...
# Rename surveys
survey_hh$Survey <- with(survey_hh, ifelse(Survey %in% "FR-CGFS","CGFS","IBTS"))
...
# Select columns of interest
select    <- c("HaulID","HaulDur", "ShootLat", "ShootLong", "Depth", "StatRec","DataType")
...
#---------------------------------------------------------------------------------------#
# Merge HH and HL and save 

# Merge
survey <- merge(survey_hh, survey_hl, by="HaulID")

# Check for duplicated rows
survey[duplicated(survey),]
survey <- survey[!duplicated(survey),]

# Save
save(survey, file=paste(outpath,"survey_data_1991-2019.RData",sep=""))
