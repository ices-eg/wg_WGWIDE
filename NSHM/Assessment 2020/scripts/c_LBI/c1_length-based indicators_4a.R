############################################################################################################-
# Length-based indicators (LBI) based on the commercial length frequency data in area 27.4.a
#
# Try out the analysis for area 27.4.a area. Two years of LF data and generally higher length.
#
...
for (i in years){
  ...
  # Add data from 7.d
  dat$prop          <- data[,match('27.4.a',colnames(data))]
  ...
  text(20,0.1,label=paste("ratio F/Fmsy = ",round(ratio_F_Fmsy,digits=3),sep=""),col="blue",cex=1.4)
