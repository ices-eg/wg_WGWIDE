...
LF_tab        <- read.table(paste0(inpath,'LF_all_PFA.csv'),sep=',',header=TRUE)
...
for(iYear in unique(LF_tab$year)){
  ...
  for(iArea in unique(LF_tab_sub$area)){
    ...
    write.table(LF_Array,file = paste0(inpath,'NSHM_LF_area_PFA_',as.character(iYear),'.csv'),sep = ',',row.names = FALSE)
  }
}