...
resPath   <- "NSHM/Assessment 2021/results/commercial/"
figPath   <- "NSHM/Assessment 2021/figures/commercial/"
...
data=read.csv(paste0(dataPath,"InterCatch/Ages from InterCatch/NSHM_",endYear,"_catch_table.csv"), stringsAsFactors = FALSE, sep=";")
...
data$season[data$season == endYear] <- "all"
...
UKcatches$year    <- endYear
UKcatches$season[UKcatches$season == endYear] <- "all"
...
catch=read.csv(paste0(dataPath,"canum.csv"), stringsAsFactors = FALSE, sep=";")
...
png(paste(figPath, "catch_N_at_age.png", sep=""), width=1700, heigh= 1300, units="px", pointsize=7, bg="white", res=300)
p = ggplot(data=catch, aes(x=year, y=age)) 
p = p + geom_point(aes(size=sqrt(catch)/max(catch)),alpha=0.5)
...
dat=read.csv(paste0(dataPath,"canum_area.csv"), stringsAsFactors = FALSE, sep=";")
...
png(paste(figPath, "catch_N_at_age_7d.png", sep=""), width=1700, heigh= 1300, units="px", pointsize=7, bg="white", res=300)
p = ggplot(data=dat, aes(x=year, y=age)) 
p = p + geom_point(aes(size=sqrt(VIId)/max(total)),alpha=0.5)
p = p + scale_shape(solid = FALSE) 
...
png(paste(figPath, "catch_N_at_age_other_minus_7d.png", sep=""), width=1700, heigh= 1300, units="px", pointsize=7, bg="white", res=300)
p = ggplot(data=dat, aes(x=year, y=age)) 
p = p + geom_point(aes(size=sqrt(outVIId)/max(total)),alpha=0.5)
p = p + scale_shape(solid = FALSE) 
p = p + scale_size_continuous(range=c(0,10))
p = p +  theme(legend.position = "none")
...
catch           <- read.csv(paste0(dataPath,"caton_area_cat.csv"), sep=";")
catchCountry    <- read.csv(paste0(dataPath,"InterCatch/Ages from InterCatch/NSHM_",endYear,"_catch_table.csv"), sep=";", head=T)
...
uniqueYears <- 2000:endYear # historical TAC only from 2000 in table
...
write.csv(outArray,paste(resPath,"TAC_catch_TAC_use_land_disc_discrate_2000-",endYear,".csv",sep=""),row.names = TRUE)
