...
# Set paths
dataPath <- "NSHM/Assessment 2021/results/length/"
resPath  <- "NSHM/Assessment 2021/results/length/"
figPath  <- "NSHM/Assessment 2021/figures/length/"
...
LF <- read.csv(paste0(dataPath,"NSHM_LF_area_",year,".csv"), stringsAsFactors = FALSE)
colnames(LF)[c(2:ncol(LF))] <- c("27.7.d","27.4.c")
...
p <- ggplot(LF_, aes(length_class,prop)) + 
  ...
  theme_bw() +
  labs(x="Length (cm)", y="Proportional number of fish", title=paste("NSHM length frequency catches",year)) +
  facet_wrap(~area, nrow=4)
print(p)            
ggsave(paste0("LF_",year,"_area.png"),plot = p, path = figPath, width = 5, height = 7)

# Load historic data
LF_2016 <- read.csv("NSHM/Assessment 2021/data/length/historic/NSHM_LF_area_2016.csv", stringsAsFactors = FALSE)
LF_2017 <- read.csv("NSHM/Assessment 2021/data/length/historic/NSHM_LF_area_2017.csv", stringsAsFactors = FALSE)
LF_2018 <- read.csv("NSHM/Assessment 2021/data/length/historic/NSHM_LF_area_2018.csv", stringsAsFactors = FALSE)
LF_2019 <- read.csv("NSHM/Assessment 2021/data/length/historic/NSHM_LF_area_2019.csv", stringsAsFactors = FALSE)
colnames(LF_2016)[2] <- "27.7.d"
colnames(LF_2017)[2] <- "27.7.d"
colnames(LF_2018)[c(2:ncol(LF_2018))] <- c("27.7.d","27.4.a")
colnames(LF_2019)[c(2:ncol(LF_2019))] <- c("27.7.d","27.4.a","27.4.b","27.4.c")
...
LF_2019_ <- melt(LF_2019, id.vars = "length_class", value.name = "prop", variable.name = "area")
colnames(LF_2019_)[c(2,3)] <- c("area","prop")
LF_2019_$year <- 2019

LF_all <- LF_
LF_all$year <- year
LF_all <- rbind(LF_all,LF_2019_,LF_2018_,LF_2017_,LF_2016_)
...
LF_all_4c <- subset(LF_all, area %in% "27.4.c")
p <- ggplot(LF_all_4c, aes(length_class,prop)) + 
  geom_col(width = 0.5, fill="red") + 
  theme_bw() +
  labs(x="Length (cm)", y="Proportional number of fish", title="NSHM length frequency catches 27.4.c") +
  facet_wrap(~year, ncol=1)
print(p)            
ggsave("LF_years_4c.png",plot = p, path = figPath,  width = 4, height = 5)
...
# Calculate mean length for each year in 4.c
years <- sort(unique(LF_all_4c$year))
meanLength_4c <- data.frame(year=years, length=rep(NA,length(years)))
for(i in 1:length(years)){
  dat <- subset(LF_all_4c, year %in% years[i])
  meanLength_4c[i,2] <- sum(dat$length_class * dat$prop)
}
