############################################################################################################-
# Length-based indicators (LBI) based on the commercial length frequency data in area 27.7.d
#
...
# Set paths
dataHistPath <- "NSHM/Assessment 2021/data/length/"
...
figAdvice    <- "NSHM/Assessment 2021/figures/advice/" 
advicePath   <- "NSHM/Assessment 2021/results/advice/"
...
for (i in years){
  ...
  if(i %in% years[c(1:length(years)-1)]){
    data <- read.table(paste0(dataHistPath,'historic/NSHM_LF_area_',as.character(i),".csv"),sep=',',header=TRUE,check.names=FALSE)
  } else{
  ...
  text(Linf,0.1,label=paste("ratio F/Fmsy = ",round(ratio_F_Fmsy,digits=3),sep=""),col="blue",cex=1.4)
