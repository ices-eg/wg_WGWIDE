...
yearIndexA      <- seq(assessmentYear-2,assessmentYear-1)
yearIndexB      <- seq(assessmentYear-5,assessmentYear-3)

# Load length-based indicators
lengthInd <- read.table(paste0(dataLengthPath,'NSHOM_size_Based_Indicators_&_Reference_Points_',as.character(assessmentYear-1),'.csv'),header = TRUE,sep=',')

# Load abundance index from surveys
indexSurveys <- read.table(paste0(dataModelPath,'model_fit_adults_nshm_option_B.csv'),header = TRUE,sep=',')
...
catch         <- historic_TAC[historic_TAC$country == 'all' & historic_TAC$year == adviceYear-1,]$advice_TAC
...
# Projected landings corresponding to advice
landingsProjected <- catchAdvice*(1-discardRate)
...
savePlot(paste0(figPath,"ratio_calculation_optionB"),type = "png")
