...
# Set paths
datpath <- "NSHM/Assessment 2021/results/model/"
...
colnames(newdata0) <- c("Survey",  "Year", "StatRec",  "HaulDur",  "HaulID", "ShootLon", "ShootLat", "Depth",  
                        "CPUE", "weight", "fit", "percarea","Perclow","Perchigh")
...
index.expl <- rbind(index.expl, data.frame(year=2020, cgfs=NA, ibts=NA, cgfs_low=NA, ibts_low=NA, cgfs_high=NA,
                                           ibts_high=NA, all=NA, all_low=NA, all_high=NA))

# Save
write.csv(index.expl, paste0(respath,"adult_surv_idx.csv"),row.names=F)

# Plot
ylim <- c(min(index.expl[,c("all","all_low","all_high")], na.rm=TRUE), 
          max(index.expl[,c("all","all_low","all_high")], na.rm=TRUE)) #define limits y-axis
xlim <- c(min(index.expl$year), max(index.expl$year)) #define limits x-axis
index.expl.plot <- subset(index.expl, !year %in% 2020) #exclude 2020 data for plotting

png(filename=paste0(figPath,"adult_surv_idx_boot.png"), height=1700, width=2500, units="px", pointsize=8, res=450)
plot(as.numeric(index.expl.plot$year), index.expl.plot$all,type="b",pch=19,col="grey40",ylim=ylim, xlim=xlim,
     xlab="Year", ylab="Survey index (no./hour)",cex.lab=1.3,
     main="Abundance index exploitable substock")
polygon(c(sort(unique(index.expl.plot$year)),rev(sort(unique(index.expl.plot$year)))), 
        c(index.expl.plot$all_high,rev(index.expl.plot$all_low)), lty=0, col=alpha(2, 0.5))
dev.off()
...
colnames(newdata0) <- c("Survey",  "Year", "StatRec",  "HaulDur",  "HaulID", "ShootLon", "ShootLat", "Depth",  "CPUE",
                         "weight", "fit", "percarea","Perclow","Perchigh")
...
# Add 2020 values as NA
index.juv <- rbind(index.juv, data.frame(year=2020, cgfs=NA, ibts=NA, cgfs_low=NA, ibts_low=NA, cgfs_high=NA,
                                           ibts_high=NA, all=NA, all_low=NA, all_high=NA))
...
# Plot
ylim <- c(min(index.juv[,c("all","all_low","all_high")], na.rm=TRUE), 
          max(index.juv[,c("all","all_low","all_high")], na.rm=TRUE)) #define limits y-axis
xlim <- c(min(index.juv$year), max(index.juv$year)) #define limits x-axis
index.expl.plot <- subset(index.juv, !year %in% 2020) #exclude 2020 data for plotting
