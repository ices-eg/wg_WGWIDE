...
wide.root<-file.path("//ait-pdfs.win.dtu.dk/Services/AQUA/Q2/gl-faellesdrev/m-faelles/WGWIDE")
...
a<-subset(a,year>=2017 & year<=assessmentYear-1)
...
p<-ggplot(data=tst, aes(x=year, y=CN, fill=type)) +
  ...
  xlab("") +ylab("Catch numbers (millions)") +
...
ggsave('prelim_final_cn.png', p, path=file.path(orig_wd,stock.dir,'data'), width = 11, height = 6)
...
xtab(aaa, caption=paste('Table 30. Preliminary and final catches. Change_PCT is (final-prelim)/prelim*100 ','.',sep=''), cornername='', 
     file=file.path(orig_wd,stock.dir,'res',paste(stamp,'_tab30.html',sep='')), dec=c(0,0,1))
